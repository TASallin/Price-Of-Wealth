...
public class Dungeon : MonoBehaviour {
	...
	void Start () {
		...
		if (!Areas.tutorialPlayed) {
			...
			storyPop.GetComponent<StoryPop>().Open();
		}
	}
	...
	public void ItemAfter (Item[] loot) {
		Party.ClearLoot();
		foreach (Item current in loot) {
			Party.AddLoot(current);
		}
	...
	*/
	...
	public void StatusChange (string method, int? degree) {
		...
		if (degree == null) {
			m.Invoke(Party.members[partyIndex].status, null);
		} else {
