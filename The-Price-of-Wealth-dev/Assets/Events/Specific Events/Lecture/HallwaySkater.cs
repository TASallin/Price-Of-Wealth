...
public class HallwaySkater : Event {
	...
	public override void Enact () {
	    ...
	    if (Party.GetPlayer().GetStrength() > 2) {
		    options1.AddLast(new TimedMethod(0, "CauseEvent", new object[] {new TextEvent("Your lead has enough strength to make the skater"
			    + " fly over your heads into the wall. Good riddance")}));
		} else {
	    ...
	    if (Party.GetPlayer().GetDexterity() > 2) {
		    options2.AddLast(new TimedMethod(0, "CauseEvent", new object[] {new TextEvent("Your lead's dexterity keeps the team out of the way."
			    + " As karma, the skater crashes into the wall")}));
		} else {
			Event dexBad = new Event();
			dexBad.text = "Your lead isn't very dextrous, and gets hit by the skater into the rest of the party";
			dexBad.options1 = new LinkedList<TimedMethod>();
			dexBad.options1.AddLast(new TimedMethod(0, "DamageAll", new object[] {3}));
			dexBad.options1.AddLast(new TimedMethod("Resolve"));
			dexBad.optionText1 = "3 damage to party";
			options2.AddLast(new TimedMethod(0, "CauseEvent", new object[] {dexBad}));
		}
