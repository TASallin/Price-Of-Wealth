...
public class Battle : MonoBehaviour {
	...
	void Update () {
		if (delay > 0) {
		    ...
		    if (methodQueue.Count > 0) {
				delay = ((float)methodQueue.Peek().delay) / 60;
	...
	public void Attack (bool playerTurn) {
		if  (playerTurn) {
			methodQueue.Enqueue(new TimedMethod(0, "Log", new object[] {player.ToString() + " attacks!"}));
	...
	public void DisableLeadReplace () {
		recruitMember.GetComponent<PartyMenu>().char1.interactable = false;
	}
	...
	public void NextTurn (bool isEnemy) {
		...
		if (!sprites.GetComponent<CharSprites>().GoodToGo()) {
			methodQueue.Enqueue(new TimedMethod(60, "NextTurn", new object[] {isEnemy}));
			return;
		}
		...
		if (isEnemy) {
			...
			if (running) {
				if (Party.GetPlayer().GetGooped() || Party.GetPlayer().GetAsleep() || Party.GetPlayer().GetStunned()) {
					...
					methodQueue.Enqueue(new TimedMethod(60, "Log", new object[] {"Ran away"}));
				    methodQueue.Enqueue(new TimedMethod(60, "Flee"));
				    return;
				}
			}
	...
	public void StatusCheck () {
		TimedMethod[] statuses;
			...
			if (Party.GetPlayer().GetPassing()) {
		        ...
		        methodQueue.Enqueue(new TimedMethod("MenuTest"));
	...
	public void MenuTest() {
		if (!sprites.GetComponent<CharSprites>().GoodToGo()) {
			methodQueue.Enqueue(new TimedMethod(60, "MenuTest"));
			return;
		}
		statusBars.transform.Find("Player Status").GetComponent<StatusBarP>().Check();
		statusBars.transform.Find("Enemy Status").GetComponent<StatusBarE>().Check();
	...
	public void EndTurn () {
		largeMenuHides.SetActive(true);
		methodQueue.Enqueue(new TimedMethod(0, "NextTurn", new object[] {true}));
	}
