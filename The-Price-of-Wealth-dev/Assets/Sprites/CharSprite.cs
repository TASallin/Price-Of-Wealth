...
public class CharSprite : MonoBehaviour {
	...
	void Start () {
		...
		if (player && (slot == Party.playerSlot - 1)) {
			gameObject.transform.localPosition = new Vector3(-100, 60, 1);
			if (displaced != null) {
	...
	void Update () {
		...
		if (delay < 0.15 && delay > 0) {
			charText.text = "";
			status.sprite = none;
			//statText.text = "";
		} else if (delay <= 0 && backlog.Count > 0) {
			charText.text = backlog.Dequeue();
			status.sprite = spritelog.Dequeue();
			//statText.text = spritelog.Dequeue();
			delay = 1;
		}
		if (delay > 0) {
	...
	public void LogSprite (string text, Sprite sprite) {
