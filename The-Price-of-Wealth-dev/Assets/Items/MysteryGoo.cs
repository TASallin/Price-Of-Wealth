	...
	public override TimedMethod[] Use () {
		...
		if (seed == 0) {
			...
			return new TimedMethod[] {new TimedMethod(0, "Audio", new object[] {"Slime"}),
				...
				new TimedMethod(0, "CharLogSprite", new object[] {(-1 * magnitude).ToString(), Party.enemySlot - 1, "charge", false})};
		} else if (seed == 4) {
			targ.GainGuard(-1 * magnitude);
			return new TimedMethod[] {new TimedMethod(0, "Audio", new object[] {"Slime"}),
			    new TimedMethod(60, "Log", new object[] {"The goo broke the target's guard by " + magnitude.ToString()}),
				new TimedMethod(0, "CharLogSprite", new object[] {(-1 * magnitude).ToString(), Party.enemySlot - 1, "guard", false})};
		} else if (seed == 5) {
